...
function CellValue({ value }: { value: boolean | string }) {
  if (typeof value === 'string') {
    return <span className="font-mono text-sm text-text-gray">{value}</span>;
  }
        ...
        <div className="mb-24 grid w-full max-w-3xl gap-6 md:grid-cols-2">
          ...
          <div className="flex flex-col border border-stroke bg-bg-card p-8">
            ...
            <div className="mb-6 flex items-baseline gap-1">
              <span className="font-mono text-4xl font-bold text-text-white">$0</span>
          ...
          <div className="relative flex flex-col border border-gold/50 bg-bg-card p-8">
            ...
            <div className="mb-1 flex items-baseline gap-1">
              <span className="font-mono text-4xl font-bold text-text-white">$5</span>
              <span className="font-mono text-sm text-text-dark">/ month</span>
            </div>
            <p className="mb-6 font-mono text-xs text-text-dark">or $50 / year (save 2 months)</p>
            <ul className="mb-8 flex flex-1 flex-col gap-3">
              {pro.map((item) => (
                <li key={item} className="flex items-start gap-3">
                  <Check size={16} className="mt-0.5 shrink-0 text-gold" />
        ...
        <div className="mb-24 w-full max-w-3xl">
          ...
          <div className="border border-stroke">
            ...
            {comparison.map((row, i) => (
              ...
              >
                <span className="font-mono text-sm text-text-gray">{row.feature}</span>
          ...
          <div className="space-y-6">
            {faqs.map((faq) => (
              <div key={faq.question} className="border-b border-stroke/50 pb-6">
                ...
                <p className="font-mono text-sm leading-relaxed text-text-gray">{faq.answer}</p>
              </div>
            ))}
          </div>
        </div>
